...
class JourneyPlanDetailsExport implements FromCollection, WithHeadings
{
    ...
    public function __construct(String $StartDate, String $EndDate, $jp_id = array())
    {
    ...
    public function collection()
    {
        ...
        if ($start_date != '' && $end_date != '') {
           ...
           //$JourneyPlan->whereRaw("DATE('created_at') BETWEEN ? AND ?", [$start_date, $end_date]);

        }

        if ($jp_id) {
            $JourneyPlan = $JourneyPlan->whereIn('id', $jp_id);
        }
       ...
       //dd($JourneyPlan);
        ...
        if (is_object($JourneyPlan)) {
            foreach ($JourneyPlan as $key => $JourneyPlan1) {
                ...
                $customers    = JourneyPlanCustomer::with('customerInfo', 'journeyPlanDay')
                    ->where('journey_plan_id', $JourneyPlan1->id)
                    ->get();
                ...
                for ($i = 1; $i <= 5; $i++) {
                    ...
                    $__week_days[$i] = $__jouney_days[$i]->pluck('day_name', 'id');
                }
