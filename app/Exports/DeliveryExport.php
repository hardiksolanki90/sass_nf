	...
	public function collection()
	{
		...
		$order = DB::table('deliveries')
			...
			->select(
				...
				'users.email',
				'orders.order_number',
				'deliveries.salesman_id',
				'deliveries.delivery_date',
				'deliveries.delivery_weight',
				'payment_terms.name as payment_term',
				'deliveries.total_qty',
				'deliveries.total_gross',
				'deliveries.total_discount_amount',
				'deliveries.total_net',
				'deliveries.total_vat',
				'deliveries.total_excise',
				'deliveries.grand_total',
				'deliveries.current_stage_comment',
				'deliveries.source',
				'deliveries.status',
				'deliveries.current_stage',
				'order_types.name as order_type',
				'deliveries.delivery_due_date',
				'items.item_name',
				'item_uoms.name as item_uom',
				'price_disco_promo_plans.name as price_disco_promo_plan',
				'delivery_details.is_free',
				'delivery_details.is_item_poi',
				'delivery_details.item_qty',
				'delivery_details.item_price',
				'delivery_details.item_gross',
				'delivery_details.item_discount_amount',
				'delivery_details.item_net',
				'delivery_details.item_vat',
				'delivery_details.item_excise',
				'delivery_details.item_grand_total',
				'delivery_details.batch_number'
			);

		if ($start_date != '' && $end_date != '') {
			$order = $order->whereBetween('deliveries.created_at', [$start_date, $end_date]);
		}
		...
		if (is_object($order)) {
			foreach ($order as $key => $ord) {
				...
				if (is_object($SalesmanInfo)) {
					$User = User::find($SalesmanInfo->user_id);
					if (is_object($User)) {
						$order[$key]->item_uom_id = $User->email;
					} else {
						$order[$key]->item_uom_id = '';
					}
				} else {
					$order[$key]->item_uom_id = '';
				}
