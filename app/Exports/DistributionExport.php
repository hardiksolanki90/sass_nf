...
class DistributionExport implements FromCollection, WithHeadings
{
	...
	public function collection()
	{
		...
		$distribution_query = DB::table('distributions')
			->select(
				'distributions.name',
				'distributions.start_date',
				'distributions.end_date',
				...
				'distribution_model_stock_details.total_number_of_facing'
			);
		...
		if (count($all_salesman)) {
			$distribution_query->join("distribution_model_stocks", function ($join) use ($all_salesman) {
				$join->on("distribution_model_stocks.distribution_id", "=", "distributions.id")->whereIn('distribution_model_stocks.customer_id', $all_salesman);
			});
		} else {

			$distribution_query->join("distribution_model_stocks", function ($join) {
				$join->on("distribution_model_stocks.distribution_id", "=", "distributions.id");
			});
		}

		$distribution_query->join("customer_infos", function ($join) {
			$join->on("customer_infos.id", "=", "distribution_model_stocks.customer_id");
		});
		...
		$distribution_query->join("distribution_model_stock_details", function ($join) {
			$join->on("distribution_model_stock_details.distribution_model_stock_id", "=", "distribution_model_stocks.id")->where('distribution_model_stock_details.is_deleted', 0)->whereNull('distribution_model_stock_details.deleted_at');
		});
