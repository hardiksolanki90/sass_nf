...
class JourneyPlanExport implements FromCollection, WithHeadings
{
	...
	public function collection()
	{
		...
		$end_date = date('Y-m-d', strtotime('+1 days', strtotime($this->EndDate)));

		$JourneyPlanQuery = JourneyPlan::select(
			'id',
			'name',
			'description',
			'start_date',
		...
		if ($start_date != '' && $end_date != '') {
			$JourneyPlanQuery->whereBetween('created_at', [$start_date, $end_date]);
		}
		$JourneyPlan = $JourneyPlanQuery->get();
					...
					if (is_object($JourneyPlanWeek)) {
						foreach ($JourneyPlanWeek as $planweek) {
							$JourneyPlanDay = JourneyPlanDay::where('journey_plan_id', $JourneyPlan[$key]->id)
								->where('journey_plan_week_id', $planweek->id)
								->get();
									...
									if (is_object($JourneyPlanCustomer)) {
										foreach ($JourneyPlanCustomer as $plancustomer) {
											...
											$JourneyPlanCollection->push((object)[
												...
												'plan_type' => $JourneyPlan[$key]->plan_type == 1 ? "Day" : "Week",
												'week_1' => $JourneyPlan[$key]->week_1 == 1 ? "Yes" : "No",
												'week_2' => $JourneyPlan[$key]->week_2 == 1 ? "Yes" : "No",
												'week_3' => $JourneyPlan[$key]->week_3 == 1 ? "Yes" : "No",
												'week_4' => $JourneyPlan[$key]->week_4 == 1 ? "Yes" : "No",
												'week_5' => $JourneyPlan[$key]->week_5 == 1 ? "Yes" : "No",
												...
												'customer_email' => (is_object($customer)) ? $customer->customer_code : '',
