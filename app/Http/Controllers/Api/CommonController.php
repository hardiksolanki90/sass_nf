...
class CommonController extends Controller
{
    ...
    public function advancedSearch(Request $request)
    {
        ...
        if ($input['module'] == "invoice") {
            ...
            $salesman = (isset($input['salesman']) ? $input['salesman'] : '');
            $storage_location_id = (isset($input['storage_location_id']) ? $input['storage_location_id'] : '');
            ...
            if (!$allData) {
                ...
                if ($storage_location_id) {
                    $selectData->where('storage_location_id', '=', $storage_location_id);
                }
        ...
        if ($input['module'] == "delivery") {
            ...
            $endrange = (isset($input['endrange']) ? $input['endrange'] : '');
            $salesman = (isset($input['salesman']) ? $input['salesman'] : '');
            $approval_status = (isset($input['approval_status']) ? $input['approval_status'] : '');
            $current_stage = (isset($input['current_stage']) ? $input['current_stage'] : '');
            ...
            if (!$allData) {
                ...
                if ($current_stage) {
                    $selectData->where('current_stage', '=', $current_stage);
                }

                if ($approval_status) {
                    $selectData->where('approval_status', '=', $approval_status);
                }
        ...
        if ($input['module'] == "order") {
            ...
            $endrange = (isset($input['endrange']) ? $input['endrange'] : '');
            $salesman = (isset($input['salesman']) ? $input['salesman'] : '');
            $customer_lpo = (isset($input['customer_lpo']) ? $input['customer_lpo'] : '');
            $delivery_date = (isset($input['delivery_date']) ? $input['delivery_date'] : '');
            $storage_location_id = (isset($input['storage_location_id']) ? $input['storage_location_id'] : '');
            $item_id = (isset($input['item_id']) ? $input['item_id'] : '');
            $user_created = (isset($input['user_created']) ? $input['user_created'] : '');
            $current_stage = (isset($input['current_stage']) ? $input['current_stage'] : '');
            $approval_status = (isset($input['approval_status']) ? $input['approval_status'] : '');
            $delivery_end_date = (isset($input['delivery_end_date']) ? $input['delivery_end_date'] : '');
            $delivery_start_date = (isset($input['delivery_start_date']) ? $input['delivery_start_date'] : '');
            ...
            $selectData = Order::with(
                ...
                'pickingSlipGenerator:id,order_id,picking_slip_generator_id,date',
            );

            if (!$allData) {
                if ($startdate) {
                    if ($startdate == $enddate) {
                        $selectData->where('order_date', "$startdate");
                    } else {
                ...
                if ($customer_lpo) {
                    $selectData->where('customer_lop', 'like', "%{$customer_lpo}%");
                }
                ...
                if ($delivery_date) {
                    $selectData->where('delivery_date', $delivery_date);
                }
                ...
                if ($item_id) {
                    $selectData->whereHas('orderDetails', function ($q) use ($item_id) {
                ...
                if ($delivery_end_date = "" && $delivery_start_date = "") {
                    if ($delivery_end_date == $delivery_start_date) {
                        $selectData->where('delivery_date', $delivery_start_date);
                    } else {
                        $selectData->whereBetween('delivery_date', [$delivery_start_date, $delivery_end_date]);
                    }
                }

                if ($user_created) {
                    $selectData->where('order_created_user_id', $user_created);
                }
                ...
                if (config('app.current_domain') == "presales") {
                    ...
                    if (count($user_branch_plant)) {
                        ...
                        $selectData->whereIn('storage_location_id', $storage_id);
                    }
                }
        ...
        if ($input['module'] == "credit_note") {
            ...
            if (!$allData) {
                if ($startdate) {
                    if ($startdate == $enddate) {
                        $selectData->where('credit_note_date', "$startdate");
                    } else {
