...
class BackMerchandiserReportController2 extends Controller
{
    ...
    private function timeSheet($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $trip = $trip_query->whereDate('trip_start', $request->start_date)->get();
        } else if ($request->end_date) {
            $trip = $trip_query->whereBetween('trip_start', [$request->start_date, $request->end_date])->get();
        } else {
            $trip = $trip_query->whereDate('trip_start', $request->start_date)->get();
        }
    ...
    private function taskAnswer($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            ...
            $start_date = $request->start_date;
            $end_date = $request->end_date;
            $survey_question_answer = $survey_question_answer_query->whereBetween('created_at', [$start_date, $end_date])->get();
    ...
    private function photos($request, $merchandiser_ids)
    {
        ...
        $distribution_post_image_query = DistributionPostImage::select('id', 'distribution_id', 'customer_id', 'salesman_id', 'image1', 'image2', 'image3', 'image4', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $distribution_post_image = $distribution_post_image_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $distribution_post_image = $distribution_post_image_query->whereDate('created_at', $request->start_date)->get();
        }
        ...
        $planogram_post_query = PlanogramPost::select('id', 'planogram_id', 'distribution_id', 'customer_id', 'salesman_id', 'description', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $planogram_post = $planogram_post_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $planogram_post = $planogram_post_query->whereDate('created_at', $request->start_date)->get();
        }
        ...
        $asset_tracking_query = AssetTrackingPost::select('id', 'asset_tracking_id', 'salesman_id', 'feedback', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $asset_tracking = $asset_tracking_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $asset_tracking = $asset_tracking_query->whereDate('created_at', $request->start_date)->get();
        }
        ...
        $campaign_picture_query = CampaignPicture::select('id', 'campaign_id', 'salesman_id', 'customer_id', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $campaign_picture = $campaign_picture_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $campaign_picture = $campaign_picture_query->whereDate('created_at', $request->start_date)->get();
        }
        ...
        $complaint_feedback_query = ComplaintFeedback::select('id', 'salesman_id', 'customer_id', 'complaint_id', 'title', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $complaint_feedback = $complaint_feedback_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $complaint_feedback = $complaint_feedback_query->whereDate('created_at', $request->start_date)->get();
        }
        ...
        $competitor_info_query = CompetitorInfo::select('id', 'salesman_id', 'company', 'item', 'price', 'brand', 'note', 'created_at')
            ...
            ->whereIn('salesman_id', $merchandiser_ids);
        if ($request->end_date) {
            $competitor_info = $competitor_info_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $competitor_info = $competitor_info_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function visitSummary($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $customer_visit = $customer_visit_query->whereDate('date', $request->start_date)->get();
        } else if ($request->end_date) {
            $customer_visit = $customer_visit_query->whereBetween('date', [$request->start_date, $request->end_date])->get();
        } else {
            $customer_visit = $customer_visit_query->whereDate('date', $request->start_date)->get();
        }
    ...
    private function orderSumamry($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $orders = $orders_query->whereDate('order_date', $request->start_date)->get();
        } else if ($request->end_date) {
            $orders = $orders_query->whereBetween('order_date', [$request->start_date, $request->end_date])->get();
        } else {
            $orders = $orders_query->whereDate('order_date', $request->start_date)->get();
        }
    ...
    private function newCustomer($request, $merchandiser_ids)
    {
        ...
        $customers_query = CustomerInfo::select('id', 'user_id', 'customer_code', 'customer_type_id', 'customer_address_1', 'customer_address_2', 'customer_city', 'customer_state', 'customer_zipcode', 'customer_address_1_lat', 'customer_address_1_lang', 'customer_address_2_lat', 'customer_address_2_lang', 'merchandiser_id', 'created_at', 'current_stage')
            ->with(
                'user:id,firstname,lastname',
        ...
        if ($request->end_date) {
            $customers = $customers_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $customers = $customers_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function competitorInfo($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $competitor_infos = $competitor_infos_query->whereDate('created_at', $request->start_date)->get();
        } else if ($request->end_date) {
            $competitor_infos = $competitor_infos_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $competitor_infos = $competitor_infos_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function orderReturn($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date) {
            $credit_note = $credit_note_query->whereBetween('credit_note_date', [$request->start_date, $request->end_date])->get();
        } else {
            $credit_note = $credit_note_query->whereDate('credit_note_date', $request->start_date)->get();
        }
    ...
    private function sos($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $shareofshelf = $sos_query->whereDate('created_at', $request->start_date)->get();
        } else if ($request->end_date) {
            $shareofshelf = $sos_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $shareofshelf = $sos_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function stockAvailability($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $stock_availability = $stock_query->whereDate('created_at', $request->start_date)->get();
        } else if ($request->end_date) {
            $stock_availability = $stock_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $stock_availability = $stock_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function taskCompleted($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            $tasks = $task_query->whereDate('created_at', $request->start_date)->get();
        } else if ($request->end_date) {
            $tasks = $task_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $tasks = $task_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function complaint($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date) {
            $complaints = $complaint_query->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        } else {
            $complaints = $complaint_query->whereDate('created_at', $request->start_date)->get();
        }
    ...
    private function routeVisit($request, $merchandiser_ids)
    {
        ...
        if ($request->end_date == $request->start_date) {
            ...
            $customer_visit = $customer_visit_query->whereBetween('date', [$request->start_date, $end_date])->get();
        } else {
            $customer_visit = $customer_visit_query->whereDate('date', $request->start_date)->get();
        }
