...
class PDPCustomer extends Model
{
    ...
    public function priceDiscoPromoPlanData()
    {
        return $this->hasMany(PriceDiscoPromoPlan::class,  'id', 'price_disco_promo_plan_id')->where('use_for', "Pricing")->where('start_date', '<=', date('Y-m-d'))->where('end_date', '>=', date('Y-m-d'))->where('status',1);
    }
}
