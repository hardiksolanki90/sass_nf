...
class CustomerLobPrd extends Model
{
	protected $table = "customer_lobs";

    protected $connection = 'server_mysql';
   // use SoftDeletes, LogsActivity, Organisationid;

    protected $fillable = [
        'uuid', 'organisation_id', 'customer_info_id', 'region_id', 'route_id', 'payment_term_id', 'lob_id', 'customer_group_id', 'sales_organisation_id', 'channel_id', 'customer_category_id', 'country_id', 'customer_type_id', 'amount', 'balance', 'credit_limit', 'credit_days', 'ship_to_party', 'sold_to_party', 'payer', 'bill_to_payer', 'merchandiser_id', 'due_on'
    ];
    ...
    {
        return $this->belongsTo(CustomerInfo::class, 'ship_to_party', 'customer_code');
    }

    public function soldToParty()
    {
        return $this->belongsTo(CustomerInfo::class, 'sold_to_party', 'customer_code');
    }

    public function payer()
    {
        return $this->belongsTo(CustomerInfo::class, 'payer', 'customer_code');
    }

    public function billToPayer()
    {
        return $this->belongsTo(CustomerInfo::class, 'bill_to_payer', 'customer_code');
    }
    ...
    public function customerBlockTypes()
    {
        return $this->hasMany(CustomerBlockType::class, 'customer_lob_id', 'id');
    }

    public function customerRoute()
    {
        return $this->hasMany(CustomerRoute::class, 'customer_lob_id', 'id')->where('is_lob', 1);
    }

    public function cusotmerWarehouseMapping()
    {
        return $this->belongsTo(CustomerWarehouseMapping::class, 'customer_info_id', 'customer_info_id');
            // ->where('customer_info_id', '=', $this->customer_info_id);
    }
