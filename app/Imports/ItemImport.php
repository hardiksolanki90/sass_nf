...
class ItemImport implements ToModel, WithValidation, SkipsOnFailure, SkipsOnError, WithMapping
{
    ...
    final public function map($row): array
    {
        ...
        $Brand_key                  = '16';
        $Sub_Brand_key              = '17';
        $Secondary_UOM_key          = '18';
        $Secondary_Item_UPC_key     = '19';
        $Secondary_UOM_Price_key    = '20';
        $Item_Stock_Keeping_Unit_key = '21';
        $Status_key                 = '22';
        $Promotional_key            = '23';
        $Product_Catalog_key        = '24';
        $Item_image_key             = '25';
        $Net_Weight_key             = '26';
        $Flawer_key                 = '27';
        $Shelf_File_key             = '28';
        ...
        $map = [
            ...
            '16' => isset($row[$Brand_key]) ? $row[$Brand_key] : "", //Group Name
            '17' => isset($row[$Sub_Brand_key]) ? $row[$Sub_Brand_key] : "", //Group Name
            '18' => isset($row[$Secondary_UOM_key]) ? $row[$Secondary_UOM_key] : "", //Group Name
            '19' => isset($row[$Secondary_Item_UPC_key]) ? $row[$Secondary_Item_UPC_key] : "", //Group Name
            '20' => isset($row[$Secondary_UOM_Price_key]) ? $row[$Secondary_UOM_Price_key] : "", //Group Name
            '21' => isset($row[$Item_Stock_Keeping_Unit_key]) ? $row[$Item_Stock_Keeping_Unit_key] : "", //Group Name
            '22' => isset($row[$Status_key]) ? $row[$Status_key] : "", //Group Name
            '23' => isset($row[$Promotional_key]) ? $row[$Promotional_key] : "", //Group Name
            '24' => isset($row[$Product_Catalog_key]) ? $row[$Product_Catalog_key] : "", //Group Name
            '25' => isset($row[$Item_image_key]) ? $row[$Item_image_key] : "", //Group Name
            '26' => isset($row[$Net_Weight_key]) ? $row[$Net_Weight_key] : "", //Group Name
            '27' => isset($row[$Flawer_key]) ? $row[$Flawer_key] : "", //Group Name
            '28' => isset($row[$Shelf_File_key]) ? $row[$Shelf_File_key] : "", //Group Name
            '29' => isset($row[$Ingredients_key]) ? $row[$Ingredients_key] : "", //Group Name
            '30' => isset($row[$Energy_key]) ? $row[$Energy_key] : "", //Group Name
            '31' => isset($row[$Fat_key]) ? $row[$Fat_key] : "", //Group Name
            '32' => isset($row[$Protein_key]) ? $row[$Protein_key] : "", //Group Name
            '33' => isset($row[$Carbohydrate_key]) ? $row[$Carbohydrate_key] : "", //Group Name
            '34' => isset($row[$Calcium_key]) ? $row[$Calcium_key] : "", //Group Name
            '35' => isset($row[$Sodium_key]) ? $row[$Sodium_key] : "", //Group Name
            '36' => isset($row[$Potassium_key]) ? $row[$Potassium_key] : "", //Group Name
            '37' => isset($row[$Crude_Fibre_key]) ? $row[$Crude_Fibre_key] : "", //Group Name
            '38' => isset($row[$Vitamin_key]) ? $row[$Vitamin_key] : "", //Group Name
            '39' => isset($row[$Catalog_Image_key]) ? $row[$Catalog_Image_key] : "", //Group Name
            '40' => isset($row[$erp_code]) ? $row[$erp_code] : "", //Group Name
            '41' => isset($row[$Lob_key]) ? $row[$Lob_key] : "", //LOB Name
        ];
        //print_r($map);
        return $map;
    }
    ...
    public function customValidationMessages()
    {
        return [
            ...
            '21.required' => 'Item Status required',
            '22.required' => 'Promotional required',
            '23.required' => 'Product Catalog required',
            // '23.required' => 'Net Weight required',
            // '24.required' => 'Flawer required',
            // '25.required' => 'Shelf File required',
