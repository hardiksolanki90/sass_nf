    ...
    public function up()
    {
        if (!Schema::hasTable('debit_note_listingfee_shelfrent_rebatediscount_details')) {
            Schema::create('debit_note_listingfee_shelfrent_rebatediscount_details', function (Blueprint $table) {
                ...
                $table->decimal('item_gross', 18, 2)->default('0.00');
                $table->decimal('item_discount_amount', 18, 2)->default('0.00');
                $table->decimal('item_net', 18, 2)->default('0.00')->comment('total_gross - total_discount_amount');
                $table->decimal('item_vat', 18, 2)->default('0.00');
                $table->decimal('item_excise', 18, 2)->default('0.00');
                $table->decimal('item_grand_total', 18, 2)->default('0.00')->comment(' total_net + total_vat');
